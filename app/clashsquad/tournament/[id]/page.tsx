...
export default function TournamentPage() {
  ...
  const [duoTeams, setDuoTeams] = useState<DuoTeamData[]>([]);
  ...
  async function loadParticipants() {
    ...
    try {
      ...
      // Duo uses the existing My Team API.
      // Do NOT query duo_teams directly from the browser because RLS protects it.
      if (isDuoTournament) {
        const response = await fetch(
          `/api/tournaments/duo/my-team?tournamentId=${encodeURIComponent(
            tournament.id
          )}`,
          {
            method: "GET",
        ...
        if (!response.ok || !result?.success) {
          throw new Error(
            result?.error || "Unable to load your Duo team."
          );
        }

        setDuoTeams(
          result?.team
            ? [result.team as DuoTeamData]
            : []
        );
      } else {
        setDuoTeams([]);
  ...
  const tournamentRules =
    Array.isArray(tournament.rules) && tournament.rules.length > 0
      ...
      : [
          ...
          "ANY TYPE OF GRENADES ARE NOT ALLOWED IN CLASH SQUAD MATCHES.",
        ];
        ...
        <div className="fixed inset-0 z-[115] flex items-end justify-center bg-black/60 px-2 pb-2 backdrop-blur-sm sm:items-center sm:px-3 sm:pb-3">
          <div className="flex max-h-[92vh] w-full max-w-md flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="bg-gradient-to-r from-[#ff174f] to-[#e91447] px-4 py-2 text-white">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-base font-black leading-tight">My Team</h3>
                  <p className="mt-0 text-[9px] font-semibold leading-3 text-white/80">
                    {isDuoTournament ? "Your Duo team details" : `${participants.length} players joined this tournament`}
                  </p>
                </div>
                <button
                  type="button"
                ...
                (() => {
                  ...
                  if (isDuoTournament) {
                    const participantByUserId = new Map(
                      participants.map((player) => [String(player.user_id).trim(), player])
                    );

                    const myDuoTeams = currentUserId
                      ? duoTeams.filter(
                          (team) =>
                            String(team.creator_user_id).trim() === String(currentUserId).trim() ||
                            String(team.member_user_id || "").trim() === String(currentUserId).trim()
                        )
                      : [];

                    const visibleTeams = myDuoTeams.filter((team) => {
                      const creator = participantByUserId.get(String(team.creator_user_id).trim());
                      const member = team.member_user_id
                        ? participantByUserId.get(String(team.member_user_id).trim())
                        : null;
                    ...
                    return visibleTeams.map((team, index) => {
                      ...
                      const creatorName =
                        creator?.users?.full_name?.trim() || team.creator_ign?.trim() || "Team Creator";
                      const creatorIgn = creator?.game_name?.trim() || team.creator_ign?.trim() || "—";
                      const creatorUid = creator?.free_fire_uid || team.creator_uid || "—";
                      const creatorLevel = creator?.users?.level ?? team.creator_level ?? "—";

                      const memberName =
                        member?.users?.full_name?.trim() || team.member_ign?.trim() || "Waiting for teammate";
                      const memberIgn = member?.game_name?.trim() || team.member_ign?.trim() || "—";
                      const memberUid = member?.free_fire_uid || team.member_uid || "—";
                      const memberLevel = member?.users?.level ?? team.member_level ?? "—";
                      const teamFull = Boolean(team.member_user_id);
                      ...
                      return (
                        ...
                        >
                          {/* TEAM HEADER */}
                          <div className="bg-gradient-to-r from-red-50 via-white to-emerald-50 px-3 py-2.5">
                            <div className="flex items-center justify-between gap-2">
                              <div className="min-w-0">
                                <div className="flex items-center gap-1.5">
                                  <span className="rounded-lg bg-[#ff174f] px-2 py-1 text-[8px] font-black text-white">
                                    DUO #{String(index + 1).padStart(2, "0")}
                                  </span>
                                  <span
                                    className={`rounded-full px-2 py-1 text-[7px] font-black uppercase ${
                                      teamFull
                                        ? "bg-emerald-100 text-emerald-700"
                                        : "bg-amber-100 text-amber-700"
                                    }`}
                                  >
                                    {teamFull ? "FULL" : "WAITING"}
                                  </span>
                                </div>
                                <h4 className="mt-1 truncate text-[13px] font-black text-gray-900">
                                  {team.team_name?.trim() || "Duo Team"}
                                </h4>
                              </div>
                          ...
                          <div className="border-t border-gray-100 px-2.5 py-2">
                            <div className="flex items-center gap-2">
                              <div className="relative shrink-0">
                                <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl border-2 border-red-100 bg-gradient-to-br from-red-50 to-emerald-50">
                                ...
                                <span className="absolute -bottom-1 -right-1 rounded-full border-2 border-white bg-red-500 px-1 py-0.5 text-[6px] font-black text-white">
                                  P1
                                </span>
                              </div>
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center justify-between gap-2">
                                  <div className="min-w-0">
                                    <p className="truncate text-[12px] font-black text-gray-900">{creatorName}</p>
                                    <p className="truncate text-[9px] font-semibold text-gray-400">{creatorIgn}</p>
                                  </div>
                                  <span className="shrink-0 rounded-full bg-red-50 px-1.5 py-1 text-[7px] font-black text-red-500">
                                    ⭐ {creatorLevel}
                                  </span>
                                </div>
                                <p className="mt-0.5 text-[9px] font-medium text-gray-400">
                                  UID: <b className="text-gray-600">{creatorUid}</b>
                                </p>
                              </div>

                              {creator && isOwnParticipant(creator) && !creator.cancelled && (
                                <button
                                  type="button"
                                  onClick={() => handleCancelTournament(creator.id, creator.user_id)}
                                  disabled={!canCancelTournament || cancelling}
                                  className={`shrink-0 rounded-lg border px-2 py-1.5 text-[7px] font-black ${
                                    canCancelTournament && !cancelling
                                      ? "border-red-200 bg-red-50 text-red-600 active:scale-[0.97]"
                                      : "cursor-not-allowed border-gray-200 bg-gray-50 text-gray-400"
                                  }`}
                                >
                                  {cancelling ? "..." : canCancelTournament ? "CANCEL" : "LOCKED"}
                                </button>
                              )}
                            </div>
                          </div>
                          ...
                          {/* PLAYER 2 */}
                          <div className="border-t border-gray-100 bg-gray-50/60 px-2.5 py-2">
                            {member ? (
                              <div className="flex items-center gap-2">
                                <div className="relative shrink-0">
                                  <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl border-2 border-emerald-100 bg-gradient-to-br from-emerald-50 to-red-50">
                                  ...
                                  <span className="absolute -bottom-1 -right-1 rounded-full border-2 border-white bg-emerald-500 px-1 py-0.5 text-[6px] font-black text-white">
                                    P2
                                  </span>
                                </div>
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center justify-between gap-2">
                                    <div className="min-w-0">
                                      <p className="truncate text-[12px] font-black text-gray-900">{memberName}</p>
                                      <p className="truncate text-[9px] font-semibold text-gray-400">{memberIgn}</p>
                                    </div>
                                    <span className="shrink-0 rounded-full bg-emerald-50 px-1.5 py-1 text-[7px] font-black text-emerald-600">
                                      ⭐ {memberLevel}
                                    </span>
                                  </div>
                                  <p className="mt-0.5 text-[9px] font-medium text-gray-400">
                                    UID: <b className="text-gray-600">{memberUid}</b>
                                  </p>
                                </div>

                                {isOwnParticipant(member) && !member.cancelled && (
                                  <button
                                    type="button"
                                    onClick={() => handleCancelTournament(member.id, member.user_id)}
                                    disabled={!canCancelTournament || cancelling}
                                    className={`shrink-0 rounded-lg border px-2 py-1.5 text-[7px] font-black ${
                                      canCancelTournament && !cancelling
                                        ? "border-red-200 bg-red-50 text-red-600 active:scale-[0.97]"
                              ...
                              <div className="flex items-center gap-2 rounded-xl border border-dashed border-amber-200 bg-amber-50 px-2.5 py-2">
                                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-lg shadow-sm">
                                  👤
                                </div>
                                <div className="min-w-0">
                                  <p className="text-[10px] font-black text-amber-700">WAITING FOR TEAMMATE</p>
                                  <p className="mt-0.5 text-[8px] font-semibold text-amber-600">
                                    Share the 6-digit team code to join this Duo team.
                                  </p>
                                </div>
                              </div>
                            )}
          ...
          <div className="max-h-[90vh] max-h-[92vh] w-full max-w-md overflow-y-auto overflow-x-hidden rounded-3xl bg-white shadow-2xl">
            ...
            <div className="max-h-[68vh] space-y-2.5 overflow-y-auto p-4">
              {[
                ...
                "DOUBLE VECTOR IS ALLOWED IN CLASH SQUAD.",
                        ...
                        <>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              maxLength={15}
                              autoComplete="off"
                              placeholder="Enter clashsquad UID"
