...
export default function CreateTournamentPage() {
  ...
  async function getNextMatchNumber() {
    ...
    for (const row of data || []) {
      const match = String(row.title || "").match(/#FF_(\d+)\b/i);
      if (match) {
  ...
  async function createTournament() {
    ...
    try {
      ...
      if (form.game === "Free Fire") {
        const nextMatchNumber = await getNextMatchNumber();
        matchId = ` #FF_${nextMatchNumber}`;
      } else if (form.game === "Free Fire MAX") {
        const { data, error } = await supabase
          .from("tournaments")
          .select("title")
          .ilike("title", "%#FFMAX_%");
        ...
        for (const row of data || []) {
          const match = String(row.title || "").match(/#FFMAX_(\d+)\b/i);
          if (match) {
            maxNumber = Math.max(maxNumber, Number(match[1]) || 0);
          }
        }

        matchId = ` #FFMAX_${maxNumber + 1}`;
      }
      ...
      <section className="card">
        ...
        <div className="prize-section">
          ...
          <div className="prize-help">
            Prize distribution is optional. You can set any amount, including ₹0.
          </div>
        </div>
