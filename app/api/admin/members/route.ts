  ...
  try {
    ...
    if (userId) {
      ...
      if (walletResult.error) {
        console.error("Member wallet error:", walletResult.error);
      ...
      if (historyResult.error) {
        console.error("Wallet history error:", historyResult.error);
        return NextResponse.json(
          { success: false, error: "Unable to load wallet history." },
          { status: 500 }
        );
      }
